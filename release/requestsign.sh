...
function uploadfile() {
  ...
  sleep 1
  curl -H "Authorization: token ${GITHUB_TOKEN}" -H "Content-Type: ${CTYPE}" --data-binary @$FILE "https://uploads.github.com/repos/vdonkey/accelerator/releases/${RELEASE_ID}/assets?name=$(basename $FILE)"
  sleep 1
}
